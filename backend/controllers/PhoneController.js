...
export const createPhone = async (req, res) => {
  const { name, code, description, brand, price, imageUrl, operatingSystem, screenSize, screenResolution, screenTechnology, refreshRate, processor, ram, storage, expandableStorage, rearCamera, frontCamera, cameraFeatures, batteryCapacity, chargingSpeed, wirelessCharging, simType, network, connectivityFeatures, waterproof, fingerprintSensor, faceRecognition, colorOptions, weight, dimensions } = req.body;

  try {
    const phone = await Phone.create({ name, code, description, brand, price, imageUrl, operatingSystem, screenSize, screenResolution, screenTechnology, refreshRate, processor, ram, storage, expandableStorage, rearCamera, frontCamera, cameraFeatures, batteryCapacity, chargingSpeed, wirelessCharging, simType, network, connectivityFeatures, waterproof, fingerprintSensor, faceRecognition, colorOptions, weight, dimensions });
    res.status(200).json(phone);
  } catch (error) {
